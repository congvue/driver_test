...
ssize_t kobject_attr_store(struct kobject* kobject, struct attribute *attr, const char *buf, size_t count)
{
	...
	if(strcmp(attr->name, "name") == 0)
		printk("Can not change the name\n");
	else if(strcmp(attr->name, "value") == 0) {
		val = buf[0] - '0';
...
{
	mod_timer(&second_devp->s_timer, jiffiess + HZ);
...
int second_open(struct inode *inode, struct file *filp)
{
	...
	add_timer(&second_devp->s_timer);// tian jia ding shi qi

	atomic_set(&second_devp->counter, 0);//ji shu qi qing ling
...
void second_exit(void)
{
	...
	unregister_chrdev_region(MKDRV(second_major, 0), 1);
