...
#include <linux/gpio.h>
     ...
     //add by zhangq
		...
		s3c_gpio_setpull(S5PV210_GPH1(6), S3C_GPIO_PULL_UP);
                irq_no = gpio_to_irq(S5PV210_GPH1(6));
		//printk("irq_no = %d\n");
                enable_irq(irq_no); 
				       ...
				       char *firmware_name)
{
	u8 *pbt_buf = NULL;
	int i_ret;
	printk("3_____fts_ctpm_fw_upgrade_with_app_file\n");
	printk("4__enter ft5x0x_GetFirmwareSize before\n");
	...
	printk("7_____fts_ctpm_fw_upgrade after");
		if (i_ret != 0)
	...
	const char __user *buff, unsigned long len, void *data)
{
	...
	printk("ft5x0x_debug_write writebuf = [%s]\n", writebuf);
	...
	case PROC_UPGRADE:
		{
			...
			DBG("%s\n", upgrade_file_path);
			//disable_irq(irq_no);
			printk("2___fts_ctpm_fw_upgrade_with_app_file before\n");
