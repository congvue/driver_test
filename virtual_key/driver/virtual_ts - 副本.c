...
#define VIR_TS_POLL_DELAY	1

static int  report_value(unsigned char *data)
{
	//msleep(VIR_TS_POLL_DELAY);
	sscanf(data, "%hd,%hd,%d",&ts_dev.x,&ts_dev.y,&ts_dev.pendown);
	printk("<kernel> buff2 = %s\n",data);

	printk(" X, Y PD = %d, %d ,%d\n",ts_dev.x, ts_dev.y, ts_dev.pendown);
	...
	if(ts_dev.pendown){
		if(ts_dev.LastState){
			...
			input_report_key(uk_dev, BTN_TOUCH, 1);
			input_report_abs(uk_dev, ABS_X, ts_dev.x);
			input_report_abs(uk_dev, ABS_Y, ts_dev.y);
			input_report_abs(uk_dev, ABS_PRESSURE, 250);
			input_sync(uk_dev);
...
ssize_t ts_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	...
	printk("<kernel> buff = %s\n",buf);

	if(count > 15){
		printk("Error data size ! data size <  11\n");
		return -1;
	}
...
static int __init ts_init(void)
{
	...
	device_create(my_class, NULL, ts_dev.devno, NULL, "virtual_tp");
