...
#define ADV739X_MODE_NTSC_UYVY8		2
#define ADV739X_MODE_PAL_UYVY8		3
...
static struct fb_videomode adv739x_modedb[] = {
	...
	{
	 /* PAL Interlaced output, include EAV/SAV */
...
#if 0
	{
	 /* NTSC Interlaced output, no EAV/SAV */
	 "UYVY8-NTSC", 60, 720, 480, 37037,
	 122, 15,
	 18, 26,
	 1, 1,
	 ...
	 FB_MODE_IS_DETAILED,},
	{
	 /* PAL Interlaced output, no EAV/SAV */
	 "UYVY8-PAL", 50, 720, 576, 37037,
	 132, 11,
	 22, 26,
	 1, 1,
...
//	printk("\n############## Line:%d ,adv7391 mode = %d\n",__LINE__,mode);
	...
	switch (mode) {
		case ADV739X_MODE_NTSC:
			...
			adv739x_write(client, 0x02, 0x30);
...
//			adv739x_write(client, 0x80, 0x10);
			...
			// Reg 0x82: SD, YPBPR
			adv739x_write(client, 0x82, 0xc1);

			adv739x_write(client, 0x83, 0x10);
...
case ADV739X_MODE_NTSC_UYVY8:
		...
		// Reg 0x00: DAC1~3 power on
		adv739x_write(client, 0x00, 0x1C);
		int data = adv739x_read(client, 0x00);
		printk("\n######## Line:%d data = 0x%2x\n",__LINE__,data);
		...
		adv739x_write(client, 0x02, 0x10);
		//adv739x_write(client, 0x02, 0x32);
		...
		// Reg 0x82: SD, CVBS
		adv739x_write(client, 0x82, 0xC9);

		
		//add by wyf
		//adv739x_write(client,0x88, 0xe0);


		// Reg 0x8A: Timing Mode 2 (slave), HSYNC/VSYNC synchronization.
		//adv739x_write(client, 0x8A, 0x0a);
		break;

#endif


		case ADV739X_MODE_PAL_UYVY8:
				printk("adv739x Func:%s Line:%d \n",__func__,__LINE__);
			// Reg 0x17: reset
			adv739x_write(client, 0x17, 0x02);
			...
			// Reg 0x8A: Timing Mode 2 (slave), HSYNC/VSYNC synchronization.
			adv739x_write(client, 0x8A, 0x0C);
			break;
...
int adv739x_fb_event(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	printk("\nadv739x Func:%s Line:%d mode->name = %s\n",__func__,__LINE__,fbi->mode->name);
		if(fbi->mode) {
			...
			else if(strcmp(fbi->mode->name, "UYVY8-NTSC") == 0)
				adv739x->cur_mode = ADV739X_MODE_NTSC_UYVY8;
			else if(strcmp(fbi->mode->name, "UYVY8-PAL") == 0)
				adv739x->cur_mode = ADV739X_MODE_PAL_UYVY8;
			adv739x_setmode(adv739x, adv739x->cur_mode);
		}
		break;
	case FB_EVENT_BLANK:
	...
	struct mxc_dispdrv_setting *setting)
{
	...
	printk("\n##############ret = %d, setting->dft_mode_str = %s\n",ret,setting->dft_mode_str);
...
static int adv739x_disp_enable(struct mxc_dispdrv_handle *disp)
{
	struct adv739x_data *adv739x = mxc_dispdrv_getdata(disp);
	adv739x_poweron(adv739x);
	return 0;
...
static void adv739x_disp_disable(struct mxc_dispdrv_handle *disp)
{
	struct adv739x_data *adv739x = mxc_dispdrv_getdata(disp);
	adv739x_poweroff(adv739x);
}
...
static struct mxc_dispdrv_driver adv739x_drv = {
	...
	.enable = adv739x_disp_enable,
	.disable = adv739x_disp_disable,
};
		...
		const struct i2c_device_id *id)
{
	...
	printk("\nokkkadv739xdaga Func:%s Line:%d i2c addr = 0x%2x \n",__func__,__LINE__
		,client->addr);
